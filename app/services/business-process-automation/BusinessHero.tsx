...
const BusinessHero: React.FC<AnimatedHeroSectionProps> = ({
  description = "Empower your business with intelligent automation solutions that optimize efficiency, streamline workflows, and drive measurable results across every department.",
  height = "min-h-screen",
}) => {
  ...
  const Stunning3DAutomationHub = () => (
    ...
    >
      ...
      { label: "ROI Analytics", icon: <HiOutlineTrendingUp />, position: "bottom-14 right-4", z: 150 },
    ].map((card, i) => (
      <motion.div
        key={i}
        animate={{ y: [0, -20, 0] }}
  ...
  return (
    <section className={`relative flex items-center overflow-hidden bg-white dark:bg-[#0a0a0a] ${height}`}>
      <canvas ref={canvasRef} className="absolute inset-0 z-0 pointer-events-none opacity-40" />
      <div className="relative lg:mt-20 z-10 container mx-auto px-2">
        <div className="flex flex-col lg:flex-row items-center gap-6">
          <div className="flex-1 max-w-3xl text-left">
            ...
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-5xl md:text-[58px] font-black text-[#073A53] dark:text-white mb-6 leading-[1.25] tracking-tighter">
  ...
  "Agile Project Execution", 
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-lg md:text-xl text-slate-500 dark:text-slate-400 mb-10 leading-relaxed font-medium">
              Empower your business with intelligent automation solutions that optimize efficiency, streamline workflows, and drive measurable results across every department.
            </motion.p>
...
<motion.button 
  ...
  onClick={() => {
    window.dispatchEvent(new Event("openNavbarDropdown"));
  }}
...
>
  ...
  <span className="inline-block ml-2 transition-transform group-hover:translate-x-1">→</span>
              ...
              <Link href="/schedule-call">
                <motion.button 
                  whileHover={{ 
                    backgroundColor: "rgba(14, 186, 176, 0.1)",
                    scale: 1.05 
                  }} 
